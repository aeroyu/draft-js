...
var styleFromNodeAttributes = function styleFromNodeAttributes(node, style) {
  ...
  return style.withMutations(function (style) {
    ...
    if (color) {
      style.add("color-".concat(color.replace(/\s/g, '')));
    }

    if (bgcolor) {
      style.add("bgcolor-".concat(bgcolor.replace(/\s/g, '')));
    }
  });
};
/**
...
for (var i = 1; i <= 12; i++) {
  PASTE_CLASS_MAPPING["rdw-textIndent-block-".concat(i)] = {
    key: 'text-indent',
    value: i
  };
}
...
var ContentBlocksBuilder = /*#__PURE__*/function () {
  ...
  _proto.trimBlockConfigs = function trimBlockConfigs(blockConfigs) {
    var _iterator = _createForOfIteratorHelper(blockConfigs),
        _step;
  ...
  _proto.addDOMNode = function addDOMNode(node) {
    ...
    this.trimBlockConfigs(this.blockConfigs); // for chaining
  ...
  _proto._toBlockConfigs = function _toBlockConfigs(nodes, style, data) {
    var _this = this;
    ...
    var _loop = function _loop(_i) {
      ...
      if (nodeName === 'body' || isListNode(nodeName)) {
        ...
        _this._trimCurrentText();

        if (_this.currentText !== '') {
          blockConfigs.push(_this._makeBlockConfig());
        } // body, ol and ul nodes are ignored, but their children are inlined in
        // the parent block config.


        var wasCurrentDepth = _this.currentDepth;
        var wasWrapper = _this.wrapper;

        if (isListNode(nodeName)) {
          _this.wrapper = nodeName;
        ...
        blockConfigs.push.apply(blockConfigs, _this._toBlockConfigs(Array.from(node.childNodes), style, new Map()));
        _this.currentDepth = wasCurrentDepth;
        _this.wrapper = wasWrapper;
        return "continue";
      }

      var blockType = _this.blockTypeMap.get(nodeName); // 图片、分割线、表格都拷贝失败，在这里过滤掉
      // - 此处不需要再过滤
      // if (blockType === 'atomic') {
      //   continue;
      // }
      ...
      if (blockType !== undefined) {
        ...
        var _wasCurrentDepth = _this.currentDepth;
        var _wasWrapper = _this.wrapper;
        _this.wrapper = nodeName === 'pre' ? 'pre' : _this.wrapper;

        if (typeof blockType !== 'string') {
          blockType = _this.disambiguate(nodeName, _this.wrapper, node) || blockType[0] || 'unstyled';
        }

        if (!experimentalTreeDataSupport && isHTMLElement(node) && (blockType === 'unordered-list-item' || blockType === 'ordered-list-item')) {
          var htmlElement = node;
          _this.currentDepth = getListItemDepth(htmlElement, _this.currentDepth);
        }

        if (node.classList.length > 0) {
          Object.keys(PASTE_CLASS_MAPPING).forEach(function (k) {
            if (node.classList.contains(k)) {
              var mappingData = PASTE_CLASS_MAPPING[k];
        ...
        var childConfigs = _this._toBlockConfigs(Array.from(node.childNodes), style, newData);
        ...
        _this.currentDepth = _wasCurrentDepth;
        _this.wrapper = _wasWrapper;
        return "continue";
      }
      ...
      if (nodeName === '#text') {
        _this._addTextNode(node, style);

        return "continue";
      }

      if (nodeName === 'br') {
        _this._addBreakNode(node, style);

        return "continue";
      }

      if (nodeName === 'hr') {
        _this._addHrNode(node, style);

        return "continue";
      }

      if (isValidFile(node)) {
        _this._addFileNode(node, style);

        return "continue";
      }

      if (isValidTable(node)) {
        _this._addTableNode(node, style);

        return "continue";
      }

      if (isValidImage(node)) {
        _this._addImgNode(node, style);

        return "continue";
      }

      if (isValidAnchor(node)) {
        _this._addAnchorNode(node, blockConfigs, style);
      ...
      blockConfigs.push.apply(blockConfigs, _this._toBlockConfigs(Array.from(node.childNodes), newStyle));
    };
  ...
  */
    ...
    Array(row).fill(0).forEach(function (_, index) {
      ...
      for (var indexCol = 0; indexCol < column; indexCol++) {
        ...
        if (tdRoot && tdRoot.rowSpan && tdRoot.colSpan && (tdRoot.rowSpan > 1 || tdRoot.colSpan > 1)) {
          for (var _i2 = 0; _i2 < tdRoot.rowSpan; _i2++) {
            if (_i2 === 0) {
              for (var j = 1; j < tdRoot.colSpan; j++) {
                if (trList[index + _i2][indexCol + j]) {
                  if (trList[index + _i2][indexCol + j].colSpan !== 0 || trList[index + _i2][indexCol + j].rowSpan !== 0) {
                    trList[index + _i2].splice([indexCol + j], 1, null, trList[index + _i2][indexCol + j]);
                  }
                } else {
                  trList[index + _i2].push(null);
                }
              }
            } else {
              for (var _j = 0; _j < tdRoot.colSpan; _j++) {
                if (trList[index + _i2][indexCol + _j]) {
                  if (trList[index + _i2][indexCol + _j].colSpan !== 0 || trList[index + _i2][indexCol + _j].rowSpan !== 0) {
                    trList[index + _i2].splice([indexCol + _j], 1, null, trList[index + _i2][indexCol + _j]);
                  }
                } else {
                  trList[index + _i2].push(null);
                }
  ...
  _proto._addHrNode = function _addHrNode(node, style) {
    this.contentState = this.contentState.createEntity('divider', 'IMMUTABLE', {});
    this.currentEntity = this.contentState.getLastCreatedEntityKey();
  ...
  _proto._toContentBlocks = function _toContentBlocks(blockConfigs) {
    ...
    for (var _i3 = 0; _i3 <= l; _i3++) {
      var config = blockConfigs[_i3];
      config.parent = parent;
      config.prevSibling = _i3 > 0 ? blockConfigs[_i3 - 1].key : null;
      config.nextSibling = _i3 < l ? blockConfigs[_i3 + 1].key : null;
  ...
  _proto._hoistContainersInBlockConfigs = function _hoistContainersInBlockConfigs(blockConfigs) {
    ...
    var hoisted = List(blockConfigs).flatMap(function (blockConfig) {
      ...
      return _this2._hoistContainersInBlockConfigs(blockConfig.childConfigs);
    });
    return hoisted;
  } // ***********************************************************************
  ...
  _proto._toFlatContentBlocks = function _toFlatContentBlocks(blockConfigs) {
    ...
    cleanConfigs.forEach(function (config) {
      var _this3$_extractTextFr = _this3._extractTextFromBlockConfigs(config.childConfigs),
          text = _this3$_extractTextFr.text,
          characterList = _this3$_extractTextFr.characterList;

      _this3.contentBlocks.push(new ContentBlock(_objectSpread(_objectSpread({}, config), {}, {
